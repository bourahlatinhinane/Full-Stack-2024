...
public static function createUser(User $user) : void {       
    $bdd = new PDO('mysql:host=localhost;dbname=game_of_fraudes;charset=utf8mb4', 'root', '');
    $sql = "INSERT INTO user (NAME_USER, FIRSTNAME_USER, TEL, MAIL, PASSWORD, adresse, code_Postal, ville) VALUES (?,?,?,?,?,?,?,?)";
    $stmt= $bdd->prepare($sql);
    $stmt->execute([$user->getNameUser(),$user->getFirstnameUser(),$user->getTelUser(),$user->getMailUser(), $user->getPasswordUser(), $user->getAdresse(), $user->getCodePostal(), $user->getVille()]);
}
/** @author Mathilde <mathilde.brx@gmail.com>*/
